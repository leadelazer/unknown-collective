// Auto-generated – run: node src/data/sync-all.js
// Do not edit directly – edit source files in src/data/encounters/<id>.md

export const ENCOUNTERS = [
  {
    "id": "curator--duchess",
    "title": "The Induction — Autumn in the Collective Hall, 1720",
    "image": null,
    "participants": [
      "curator",
      "duchess"
    ],
    "body": "The vote was held in the third-floor room with the low ceiling and the crooked shutter. Seven members were present. The Duchess had counted them before she arrived: she knew which two would abstain, which one would speak too long, and which would watch her face instead of the candidate. What she had not counted on was how long Freddy would stand without speaking when they finally let him in.\n\nHe stood at the far end of the table, coat damp from the street, hands at his sides. He had been a baker's son and then a soldier and then a scholar of law, and the old guard looked at him the way they looked at a street map they couldn't quite reconcile with their memory of the city. One of them — she would not record his name in any account — called him an instrument of the Duchess's sentiment. The word landed like a coin on stone.\n\nShe did not answer it. She passed three folded pages across the table instead: the argument for his induction written in the language of the archive, impersonal and precise. She had been preparing it for eleven months. The document cited no personal connection. It cited only the nature of the Bind, and the kind of mind required to map what it was doing to the city's records.\n\nHe was voted in by five to two. The Duchess closed her portfolio and said nothing further. Later that evening, in the corridor, Freddy asked her why she had not spoken in his defence. She looked at him for a moment. *The document was the defence,* she said. *A spoken argument they could dismiss as feeling. A written one they have to file.*",
    "hasContent": true
  },
  {
    "id": "curator--florist--gardener--oracle",
    "title": "The Assembly — A Room above the Canal, Autumn 1892",
    "image": null,
    "participants": [
      "curator",
      "florist",
      "gardener",
      "oracle"
    ],
    "body": "The Oracle had chosen the room carefully. She had described it to each of them differently: to the Gardener as a quiet space above the water where they could speak without interruption; to the Florist as the place where the city looked most like itself from above; to the Curator as a room with good light and a table large enough for documents. None of these descriptions was untrue. None of them was the whole reason.\n\nShe had left the von Waldeck estate three years earlier with a cold head and a list of names. She had taken her time. She had watched each of them operate in their own domain — the Curator in his archive, the Florist in her shop in the old quarter, the Gardener in the estate gardens and then in the market gardens beyond the western gate. She had identified what each of them could see that the others could not, and she had worked out what the four of them together could see that no one else in the Collective was currently seeing.\n\nThey arrived at different hours. By the time all four were seated, the canal below had gone dark and the lamplight made the room into its own weather system. The Oracle laid nothing on the table. She spoke for perhaps six minutes: a short account of the Bind's behaviour over the preceding decade, expressed not in terms of the archive but in terms of what had stopped growing where, what routes had been quietly closed, which kinds of knowledge had become difficult to locate in the city without being told where to look. She used the Gardener's language and the Curator's language and the Florist's language, which she had been learning for three years.\n\nNo one spoke for a moment after she finished. Then the Gardener said: *I have been watching the same thing from the soil since 1884.* The Curator said he had evidence but had not connected it to a pattern. The Florist said nothing, but she reached into her bag and produced the cloth-covered notebook — the one the Curator had annotated — and opened it to a page he had not seen. She had been keeping her own parallel record. He looked at it for a long time. Outside, the canal moved. The lamplighter went past, and the room changed colour twice.",
    "hasContent": true
  },
  {
    "id": "curator--florist--oracle",
    "title": "What the Archive Is Missing — The Cold Reading Room, November 1894",
    "image": null,
    "participants": [
      "curator",
      "florist",
      "oracle"
    ],
    "body": "The Oracle brought the Florist to the archive in November, without telling her why. The room was cold and smelled of old leather and something underneath — a chemical presence, faint, like vinegar or a pressed plant going dark. The Florist noticed it at once. She said: *Something in here is dying.* The Curator looked up from his table and said nothing.\n\nThe Oracle had been working a theory for two years. She had identified seventeen entries in the municipal archive whose references survived in correspondence and footnotes, but whose actual contents were gone — not misfiled, not damaged, simply absent in a way that felt structural rather than accidental. The pattern, when she laid it out on the long table, was unmistakable: every missing entry concerned a natural system. The river's flood records from 1698. The survey of tree canopy before the city's western expansion. The Weavers' original commission papers. Whatever the Bind had tightened around first, it had tightened around evidence of what the city had grown from.\n\nThe Florist understood it before the Curator did — or before he would admit to it. She walked the shelf line with her coat still buttoned, reading spines, and said: *The gaps are in the living things. Everything still here is stone and finance.* The Curator said the gaps had other explanations. The Oracle said, without raising her voice: *Name one.*\n\nHe named three. They argued for an hour. By the end, nothing was resolved, but the Curator had begun writing in his notebook again — the kind of writing, the Oracle recognised, that meant he had shifted from defending a position to examining it. When they left, the Florist paused in the doorway and placed a single lavender stem on the reading table. Devotion, silence, and distrust. He found it still there the following morning, and did not move it for eleven days.",
    "hasContent": true
  },
  {
    "id": "curator--florist",
    "title": "The Dictionary — Estate Gardens, Summer 1893",
    "image": null,
    "participants": [
      "curator",
      "florist"
    ],
    "body": "He had walked the estate gardens for an hour before he noticed her. She was on the far terrace, building an arrangement from what looked like ordinary garden cuttings — lavender, late roses, a stem of foxglove the gardeners had missed. He watched from a distance and told himself he was inspecting the hedgerows.\n\nWhen he did approach, it was to criticise. He said the foxglove was wrong for the occasion, that its meaning in the standard floriographic canon was insincerity, and that someone had probably sent it deliberately. She looked at him steadily and said the standard canon was out of date in four regions and that he should know that, if he read what he claimed to read. He said nothing. She handed him the arrangement and asked him to hold it while she reached for her notebook.\n\nThe notebook was a small cloth-covered thing, densely annotated in two different inks. It was not, he understood quickly, a copy of anything published. It was the working record of her own system — the variant readings she had collected from flower-sellers across three cities, cross-referenced against occasion and recipient and outcome. He read three pages before she looked up and found him doing so.\n\nShe did not ask for it back immediately. He read four more pages. Later, he would catalogue the dictionary in the archive without asking her permission. He added his own annotations in a third hand. When she discovered this — and she did discover it, immediately — she said only: *At least your corrections are accurate.* He kept a copy. The original remains filed under her name.",
    "hasContent": true
  },
  {
    "id": "curator--gardener--mapmaker--oracle",
    "title": "The Street That Changes — Surveyor's Office, March 1895",
    "image": null,
    "participants": [
      "curator",
      "gardener",
      "mapmaker",
      "oracle"
    ],
    "body": "The Mapmaker had been watching a particular street for four years before he brought it to the others. It appeared on his current-edition map at the correct position: a short connecting lane between the river quarter and the street of tanners, paved, with two street lamps and a well-attested bakehouse at its south end. When you walked to where it should be, you found a blank wall and an alley that went nowhere. He had checked the municipal survey. He had checked his instruments. He had walked the route himself on six separate occasions across three seasons.\n\nThe Curator came because the file discrepancy was in his archive. The Oracle came because she had been building a case. The Gardener came because she had been tending a strip of ground near the river quarter for two years and had twice found that the plants she'd put in the previous season were gone without record of removal — not dead, not overgrown, simply absent, with the soil undisturbed.\n\nThe Mapmaker spread four versions of his map across the survey table, spanning from 1872 to the current year. In 1872, the lane existed. By 1881, it was present on the map but absent in person. By 1887, it had begun to appear on the map in different positions in different copies of the same edition. The Curator said this was a printing error. The Mapmaker said he had sourced the maps from three different printers. The Oracle said the discrepancy was concentrated along the line of the old canal tributary — the waterway the Weavers had built their studio above before it was paved over in 1699. The room was quiet for a moment.\n\nThe Gardener went to the wall and ran her finger along a crack in the plaster. She said: *This building is on that line too.* They all looked. The crack ran from the floor to the ceiling in a perfectly straight vertical path, and it was warm to the touch.",
    "hasContent": true
  },
  {
    "id": "curator--merchant-prince",
    "title": "The Third Ledger — Administrative Gate, October 1879",
    "image": null,
    "participants": [
      "curator",
      "merchant-prince"
    ],
    "body": "The Merchant Prince had been a member of the Collective for less than a year when he made his first substantial offer to the archive. He proposed to fund the restoration of two entire shelves of deteriorating records, at his own cost and without conditions. The Curator accepted, publicly, in the terms the offer deserved. The private terms were not published.\n\nThe private terms required the Curator to transfer three ledgers pertaining to the history of the city's canal infrastructure — specifically, the diversion of the river tributary that had run beneath the Weavers' quarter before the urban expansion of the 1690s. The Merchant Prince was mapping expansion routes for the coming industrial decade. The canal history was material to his calculations. He was, he said, simply working with the full record. He said this in the manner of a man who had learned to say the obvious thing first and wait for what it dislodged.\n\nAt the administrative gate in October, the Curator arrived with two ledgers in a satchel. The Merchant Prince counted them. He said nothing immediately. He turned each over in his hands and read the binding stamps. Then he said: *There should be three.* The Curator said the third was under review for condition. This was true; he had placed it under review himself, the previous week, by writing a single note in the margin of its catalogue entry. The Merchant Prince looked at him for a moment. *You understand,* he said, *that the two shelves are funded regardless.* The Curator said he appreciated that. He did not hand over the third ledger.\n\nThe third ledger concerned not the canal infrastructure but the original commission of the Bind — the Weavers' formal arrangement with the Collective in 1698, including what they had agreed to provide and what the Collective had agreed never to dismantle. The Curator has never confirmed this. The ledger is listed in the archive as under ongoing review. The Merchant Prince funded three shelves instead of two the following year, without comment.",
    "hasContent": true
  },
  {
    "id": "curator--oracle",
    "title": "The Lost Ledger — Autumn in the Central Archive, 1906",
    "image": null,
    "participants": [
      "curator",
      "oracle"
    ],
    "body": "The central archive was unheated that autumn. Damp crept up the stone walls, staining ledgers stacked near the iron grating. The Curator arrived before dawn, coat buttoned, gloves missing. The Oracle entered after him, footsteps echoing in the corridor. She paused at the doorway, gaze lingering over the shelves.\n\nA ledger, dust-thick, lay open on the reading table. The Curator asked short questions. The Oracle shook her head once, then once more. She traced lines on the page without speaking. The clock on the wall slipped past seven. Outside, municipal workers swept leaves past the entryway.\n\nHe wrote her name in pencil inside the ledger, then scratched it out. She left with a slip of paper, folded three times. Later, officials noted a blank space in the records. No signature. No witness. The archive remained cold until spring.\n\nNo one commented on the ledger's absence, though its file reference persisted in correspondence for years. Questions were raised but not followed. The incident's importance was uncertain; some cited routine error, others whispered about protocol. No new policies were issued. The archive’s door, recently repaired, squeaked when closed.",
    "hasContent": true
  },
  {
    "id": "duchess--merchant-prince",
    "title": "What She Will Not Sell — The Collective Hall, Spring 1880",
    "image": null,
    "participants": [
      "duchess",
      "merchant-prince"
    ],
    "body": "He had done his reading before the meeting. That much was evident from the way he waited — not in the hall proper but in the anteroom, where the archive's older correspondence was filed, running a thumb along a shelf of bound letters. When she came in, he did not pretend to have been doing something else. She approved of this, although she did not say so.\n\nWhat he wanted was the correspondence between the Duchess and the Weavers — specifically, the letters from 1700 through 1712, which she had retained in her private collection rather than transferring them to the formal archive. The Curator knew they existed; she had told him, once, that they contained the clearest account of the Bind's first decade of behaviour. The Merchant Prince had arrived at the same conclusion through a different route. He had mapped the city's economic anomalies across the previous century and found that they clustered around the same dates and districts as the Bind's known effects. He wanted the primary sources. He explained this to her plainly, with the confidence of a man who believed that a rational case would be met with a rational response.\n\nShe let him finish. Then she said: *I know exactly what the letters contain. That is precisely why they remain where they are.*\n\nThe exchange that followed was the kind the Collective's history would later describe as legendary — though the account that survives is the Duchess's own, and she gives the Merchant Prince considerably more intelligence than the story usually affords him. She acknowledged that his economic analysis was accurate. She accepted that the information would be useful to the city. She held the velvet correspondence pouch throughout and did not open it. Her argument was not that the knowledge was too dangerous to share; it was that the Bind had survived two centuries partly because the Collective had never made it legible to the kind of mind that would try to use it. *You are exactly the kind of mind I am describing,* she said. *That is why I am glad you are here. It is also why I will not give you this.*",
    "hasContent": true
  },
  {
    "id": "florist--gardener",
    "title": "What Grows Here — The Estate Glasshouse, Early Summer 1891",
    "image": "/assets/stories/gardener-florist.png",
    "participants": [
      "florist",
      "gardener"
    ],
    "body": "Amelia had been on the estate for three weeks before she and Carla were properly alone together. The house had its routines — footsteps at certain hours, the particular silences that meant someone was listening — and so the glasshouse was useful. It was the kind of space that announced itself as productive. No one interrupted people who appeared to be working.\n\nThey were working. Amelia had been given the cut flowers from the previous morning's market order, and she was rebuilding the arrangements that the Duchess's housekeeper had assessed as too spare. Carla was repotting a row of pelargoniums that had been left too long, their roots pressing against the clay in the way that meant they had been waiting rather than growing. They did not speak for a long time. The glasshouse had that quality: it made silence seem like a reasonable activity.\n\nWhat broke it was a stem of red ranunculus. Amelia had been working around it for several minutes, placing it and removing it, and Carla had watched this without comment until she couldn't. She said: *It wants to be central.* Amelia looked up. Carla said it again, differently: not as advice but as observation — the kind of remark that comes from watching something closely enough that the thing itself seems to have an opinion. Amelia placed the ranunculus at the centre of the arrangement and did not move it again.\n\nLater, Carla would describe this as the beginning of their understanding. She would not say of what. Amelia, who kept her records in a cloth-covered notebook, made a note that evening: *ranunculus — she sees before I decide.* What Carla had noticed first, and did not yet say to Amelia, was that two of the pelargoniums nearest the north wall of the glasshouse had not grown at all since November, despite good soil and adequate light. She had no name for what was happening to them. She pressed her thumb into the earth beside the smallest pot and felt nothing wrong. That was the part that bothered her.",
    "hasContent": true
  },
  {
    "id": "gardener-oracle--merchant-prince",
    "title": "The Northern Ground — River Quarter, Late Summer 1895",
    "image": null,
    "participants": [
      "gardener",
      "oracle",
      "merchant-prince"
    ],
    "body": "In the northern district, an argument arose between the Gardener and the Merchant Prince. It transpired in a dimly lit shop filled with plants and a competing display of imported goods. The Gardener, dirt under fingernails, held a fresh bouquet. Its vibrant blooms, grown in her own patch, radiated life. The Merchant Prince examined them with a critical eye. \n\nHe dismissed the bouquet’s worth. “This cannot compete with what is harvested from abroad,” he asserted, his voice echoing among rows of potted cacti. The Gardener countered with fervor, emphasizing the essence of local production. “It is not just about currency. It’s about roots, growth, and sustainability.” \n\nThe Oracle watched quietly from the corner. Her focus was not on the flowers or the price tags, but on the tension boiling over. The weight of her silence suggested deeper implications, perhaps voicing unspoken truths about the economy of the riverbank and its effects on community values. In that moment, it was clear: their clash was not merely over flowers, but over what they stood to lose and gain in a changing world.",
    "hasContent": true
  }
];
